...
def get_rectified_flow_loss_fn(sde, train, reduce_mean=True, eps=1e-3):
  ...
  def loss_fn(model, batch):
    ...
    if sde.reflow_flag:
        ...
        print(f"DEBUG: sde.T={sde.T}, eps={eps}, batch_device={batch.device}")
        print(f'batchsize {batch.shape[0]}')
        t = (torch.rand(batch.shape[0], device="cpu", dtype=torch.float32) * (sde.T - eps) + eps).to(batch.device)
